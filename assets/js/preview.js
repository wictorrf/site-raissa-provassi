...
// "Por fuera": tapa + elástico + grabado opcional + dijes recortados sobre la tapa.
function buildExteriorSVG({ coverHex, elasticHex, engraveOn, engraveText, charmIds }) {
  ...
  const charmsMarkup = (charmIds || []).map((id, i) => {
    ...
    const x = 52 + col * 24, y = 178 + row * 26;
    return `<text x="${x}" y="${y}" font-size="19">${CHARM_EMOJI[id] || "•"}</text>`;
  }).join("");
  return `
  <svg class="journal-svg-dynamic" viewBox="0 0 220 300" style="width:150px;">
    <rect x="150" y="14" width="56" height="272" rx="7" fill="#F2EAD9"></rect>
    <rect x="30" y="10" width="150" height="280" rx="12" fill="${coverHex}"></rect>
    <circle cx="47" cy="27" r="3" fill="rgba(255,255,255,.55)"></circle>
    <circle cx="163" cy="27" r="3" fill="rgba(255,255,255,.55)"></circle>
